...
def check_and_download_new_data(date_str):
    """Check if data exists for a given date and download/process using pandas"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Calculate major sports volume
            sports_mask = df['report_ticker'].str.contains('|'.join(MAJOR_US_SPORTS), na=False)
            major_sports_volume = df.loc[sports_mask, 'daily_volume'].sum()

            # Calculate KXMVE volume
